...
####### MY TRIAL OF PLOTTING IT AS TIME SERIES
...
start = netCDF4.num2date(time[0],time.units)
istart = netCDF4.date2index(start,time,select='nearest')
hs = ws[istart,ix,iy]
tim = time[istart]

# Create Pandas time series object
ts = pd.Series(hs,index=start)
